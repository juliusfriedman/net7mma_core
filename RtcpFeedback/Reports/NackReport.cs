...
namespace Media.Rtcp.Feedback
{
    public class NackReport : TransportLayerFeedbackReport
    {
        public NackReport(int version, int padding, int ssrc, int mssrc, IEnumerable<int> lostPackets)
            : base(version, TransportLayerFeedbackReport.TransportLayerFormat.Nack, padding, ssrc, mssrc, lostPackets is not null ? new byte[Common.Binary.BytesPerInteger * lostPackets.Count()] : null)
        {
