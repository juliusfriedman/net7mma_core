...
namespace Media.Http
{
    ...
    public class HttpMessage : Common.SuppressedFinalizerDisposable, Common.IPacket
    {
        ...
        public static HttpMessage CreateMultipart(out MultipartContent multiPartcontent, byte[] boundary, System.Text.Encoding encoding = null, params Tuple<string, string, byte[], byte[]>[] contents)
        {
            ...
            //    //Disposition
            //    if (string.IsNullOrWhiteSpace(content.Item1) is false) data.AddRange(encoding.GetBytes(HttpHeaders.ContentDisposition + ":" + content.Item1));

            //    //Item2 = Content-Type
            //    if (string.IsNullOrWhiteSpace(content.Item1) is false) data.AddRange(encoding.GetBytes(HttpHeaders.ContentType + ":" + content.Item2));
        ...
        public static Uri Wildcard = new("*", UriKind.RelativeOrAbsolute);
        ...
        public static HttpMessage FromString(string data, System.Text.Encoding encoding = null)
        {
            ...
            encoding ??= HttpMessage.DefaultEncoding;
        ...
        //Todo, could consolidate storage with a key type which indicates if the header is entity or otherwise... FlaggedString{ int Flags; string Value; }
        /// <summary>
        ...
        public bool IsPersistent
        {
            ...
            get
            {
                return ShouldDispose is false;
            }
            [System.Runtime.CompilerServices.MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]
            set
            {
                ShouldDispose = value is false;
            }
        }
        ...
        public int StatusCode
        {
            ...
            set
            {
                ...
                if (CanHaveBody is false) m_Body = string.Empty;
            }
        }
        ...
        public int Length
        {
            ...
            get
            {
                ...
                try
                {
                    ...
                    Media.Common.Extensions.Array.ArrayExtensions.IsNullOrEmpty(m_EncodedLineEnds, out int lineEndsLength);

                    Media.Common.Extensions.Array.ArrayExtensions.IsNullOrEmpty(m_EncodedWhiteSpace, out int whitespaceLength);
                    ...
                    if (m_Headers.Count > 0)
                    {
                        ...
                        //Because there might be 0 headers we add lineEndsLength last again rather than with << 1
                        length += count * (1 + lineEndsLength) + lineEndsLength;
                    }

                    if (false == string.IsNullOrEmpty(m_Body))
                    {
        ...
        public Encoding HeaderEncoding
        {
            ...
            set
            {
                if (IsDisposed && IsPersistent is false) return;
                ...
                //Could also cache byte versions to skip encoding in Prepare?
                //Tuple<char[], byte[]> EncodedCharcter where the char[] is the chars and the byte[] is the bytes...
            ...
            : base(shouldDispose)
        {
            ...
            if (data is null || offset < 0 || offset >= (length = Media.Common.Binary.Clamp(length, 0, data.Length)))
            {
                return;
            }
            ...
            if (ParseStatusLine() is false) return;
        ...
        protected virtual bool ParseStatusLine(bool force = false)
        {
            ...
            if ((sawDelemit = Media.Common.Extensions.Encoding.EncodingExtensions.ReadDelimitedDataFrom(HeaderEncoding, m_Buffer.GetBuffer(), m_EncodedLineEnds, m_Buffer.Position, m_Buffer.Length, out StatusLine, out read, true)) is false &&
                ...
                case -1:
                    {
                        ...
                        //Todo, Option AllowInvalidStatusProtocol
                        break;
                    }
        ...
        protected virtual bool ParseHeaders(bool force = false)
        {
            try
            {
                ...
                if (m_Buffer is null || m_Buffer.CanSeek is false) return false;
                        ...
                        (remains = max - position) > 0)
                {
                    ...
                    sawDelemit = Media.Common.Extensions.Encoding.EncodingExtensions.ReadDelimitedDataFrom(HeaderEncoding, buffer, m_EncodedLineEnds, position, remains, out string rawLine, out justRead, true);
                ...
                //if (sawDelemit is false || emptyLine >= 2)
                //{
                //    position -= justRead;
                //}
        ...
        protected virtual bool ParseTrailer()
        {
            try
            {
                ...
                {
                    //Determine if any of the delimits were found
                    sawDelemit = Media.Common.Extensions.Encoding.EncodingExtensions.ReadDelimitedDataFrom(HeaderEncoding, m_Buffer, m_EncodedLineEnds, remains, out string rawLine, out justRead, out Exception encountered, true);
        ...
        protected internal virtual bool ParseBody(out int remaining, bool force = false) //bool requireContentLength = true
        {
            ...
            if (available > 0)
            {
                ...
                if (Version < 2.0 && Version > 1.0 && GetHeader(HttpHeaders.TransferEncoding) == "chunked")
                {
                    int chunkSize = -1;
                    ...
                    while (available > 0 && (chunkSize = ParseChunk(ref position, available)) >= 0)
                    {
                        ...
                        if (available > 0 && Array.IndexOf<char>(m_EncodedLineEnds, decoder.GetChars(buffer, position, 1)[0]) >= 0)
                        {
                            ++position;
        ...
        internal string GetHeaderValue(string name, out string actualName)
        {
            ...
            foreach (string headerName in GetHeaders())
                if (string.Compare(name, headerName, true) is 0) //headerName.Equals(name, StringComparison.OrdinalIgnoreCase);
                {
                    ...
                    if (m_Headers.TryGetValue(actualName, out string result)) return result;
                }
            return null;
        }
        ...
        internal string GetEntityHeaderValue(string name, out string actualName)
        {
            ...
            foreach (string headerName in GetEntityHeaders())
                if (string.Compare(name, headerName, true) is 0) //headerName.Equals(name, StringComparison.OrdinalIgnoreCase);
                {
                    ...
                    if (m_EntityHeaders.TryGetValue(actualName, out string result)) return result;
                }
            return null;
        }
        ...
        public virtual void SetHeader(string name, string value)
        {
            ...
            if (AllowInvalidHeaders is false &&
                char.IsLetter(name[0]) is false) return; // || false == string.IsNullOrWhiteSpace(value) && CountChars(value, m_StringColon) > 1, throw InvalidOperationException()
            ...
            if (ContainsHeader(name, out string actualName)) m_Headers[actualName] = value;
            else
            {
        ...
        {
            //If the name is no name then the value is not relevant
            if (IsDisposed && IsPersistent is false || string.IsNullOrWhiteSpace(name)) return;
            ...
            //If the header with the same name has already been encountered set the value otherwise add the value
            if (ContainsEntityHeader(name, out string actualName)) m_EntityHeaders[actualName] = value;
            else
            {
        ...
        public virtual void AppendOrSetHeader(string name, string value) //string joiner
        {
            ...
            if (AllowInvalidHeaders is false &&  // || false == char.IsLetterOrDigit(value[0])
                char.IsLetter(name[0]) is false) // || false == string.IsNullOrWhiteSpace(value) && CountChars(value, m_StringColon) > 1, throw InvalidOperationException()
            {
                //If not return
                return;
            }
            ...
            if (ContainsHeader(name, out string containedHeader) is false) SetHeader(name, value); //joiner ?? 
            else if (nullValue is false)
            {
        ...
        public virtual void AppendOrSetEntityHeader(string name, string value) //string joiner
        {
            ...
            //If not already contained then set, otherwise append the value given
            if (ContainsEntityHeader(name, out string containedHeader) is false) SetEntityHeader(name, value); //joiner ?? 
            else if (nullValue is false)
            {
                string containedValue = m_Headers[containedHeader];
        ...
        public bool TryGetBuffers(out System.Collections.Generic.IList<System.ArraySegment<byte>> buffer)
        {
            ...
            if (m_Buffer.TryGetBuffer(out ArraySegment<byte> existingBuffer))
            {
        ...
        {
            if (IsDisposed && IsPersistent is false) return Common.MemorySegment.EmptyBytes;
        ...
        /// <returns></returns>
        public virtual IEnumerable<byte> PrepareStatusLine(bool includeEmptyLine = true, bool escapeUri = true, Encoding uriEncoding = null)
        {
            IEnumerable<byte> sequence = Common.MemorySegment.Empty;

            if (IsDisposed && IsPersistent is false) return sequence; //yield break;

            System.Text.Encoding headerEncoding = m_HeaderEncoding;

            if (headerEncoding is null) return sequence;
            ...
            if (MessageType is HttpMessageType.Request or HttpMessageType.Invalid)
            {
                ...
                if (escapeUri) uri = System.Uri.EscapeUriString(uri); //Converts to Unicode to Utf8 where needed so set to false if utf16 or otherwise is needed.

                sequence = Enumerable.Concat(sequence, uriEncoding.GetBytes(uri)); //Use the encoding on the escaped string with (now utf8 chars if encodeUri was true).
        ...
        public virtual IEnumerable<byte> PrepareHeaders(bool includeEmptyLine = true)
        {
            ...
            if (IsDisposed && IsPersistent is false) return sequence;
            ...
            if (m_EncodedWhiteSpace is null) return sequence;
        ...
        public virtual IEnumerable<byte> PrepareBody(bool includeEmptyLine = false)
        {
            ...
            if (IsDisposed && IsPersistent is false || CanHaveBody is false) return sequence;

            if (string.IsNullOrWhiteSpace(m_Body) is false)
            {
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            base.Dispose(ShouldDispose && IsPersistent is false);
        ...
        {
            return Created.GetHashCode() ^ ((int)MessageType | (int)HttpMethod ^ (int)HttpStatusCode) ^ (string.IsNullOrWhiteSpace(m_Body) ? Length : m_Body.GetHashCode()) ^ (m_Headers.Count);
        }
        ...
        public static bool operator !=(HttpMessage a, HttpMessage b) { return (a == b) is false; }
        ...
        public virtual int CompleteFrom(System.Net.Sockets.Socket socket, Common.MemorySegment buffer)
        {
            ...
            if (hasSocket)
            {
                ...
                if (ParseBody(out int remaining, false) is false && remaining > 0)
                {
                    //Store the error
        ...
        private int ParseChunk(ref int offset, int length) //out string extensions
        {
            ...
            //If it was not present then do not parse further
            if ((sawDelemit = Media.Common.Extensions.Encoding.EncodingExtensions.ReadDelimitedDataFrom(HeaderEncoding, m_Buffer, m_EncodedLineEnds, length, out string ChunkLine, out long read, true)) is false)
            {
                return -1;
            }
            ...
            if (parts.Length > 0)
            {
                ...
                if (int.TryParse(parts[0], System.Globalization.NumberStyles.HexNumber, System.Globalization.CultureInfo.InvariantCulture, out ChunkLength) is false) return -1;
...
namespace Media.UnitTests
{
    ...
    internal class HttpMessgeUnitTests
    {
        public void TestRequestsSerializationAndDeserialization()
        {
            ...
            {
                using (Media.Http.HttpMessage request = new(Media.Http.HttpMessageType.Request))
                {
                    ...
                    using (Media.Http.HttpMessage serialized = new(bytes))
                    {
        ...
        public void TestResponsesSerializationAndDeserialization()
        {
            ...
            {
                using (Media.Http.HttpMessage response = new(Media.Http.HttpMessageType.Response)
                {
                    Version = 7,
